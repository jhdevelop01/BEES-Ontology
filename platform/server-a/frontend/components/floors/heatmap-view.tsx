        ...
        return (
          <button
            ...
            className={`
              ...
              focus-visible:ring-cyan-400 focus-visible:ring-offset-1 focus-visible:ring-offset-slate-950
              ${statusBorderColor(floor.status)}
            `}
          >
            {/* ---- Floor label ---- */}
            <span className="w-[3rem] shrink-0 text-xs font-semibold text-slate-200 text-right pr-1">
              {floor.label}
            </span>

            {/* ---- Temperature bar (heatmap core) ---- */}
            <div className="flex-1 relative h-7 rounded overflow-hidden bg-white/5">
              {hasData ? (
                <>
                  {/* Coloured bar — full width, colour encodes temperature */}
                  <div
                    className="absolute inset-0 rounded transition-colors duration-500 opacity-80"
                    style={{ backgroundColor: bgHex }}
                  />

                  {/* Temperature value overlay */}
                  <span className="relative z-10 flex items-center h-full px-2 text-xs font-medium text-white/90">
                    {floor.temperature!.toFixed(1)}{"\u00B0"}C
                  </span>
                </>
              ) : (
                <span className="relative z-10 flex items-center h-full px-2 text-xs text-slate-500 italic">
                  {t("noData")}
                </span>
              )}
            </div>
            ...
            {/* ---- Equipment count ---- */}
            <span className="w-[4rem] shrink-0 text-xs text-slate-400 tabular-nums text-center">
            ...
            {/* ---- Power ---- */}
            <span className="w-[4.5rem] shrink-0 text-xs text-slate-400 tabular-nums text-right">
              {floor.powerKw !== null ? (
                <>
                  {floor.powerKw < 10
                    ? floor.powerKw.toFixed(1)
                    : Math.round(floor.powerKw)}
                  <span className="text-slate-500 ml-0.5">kW</span>
                </>
              ) : (
                <span className="text-slate-600">&mdash;</span>
              )}
            ...
            <span className="w-[2rem] shrink-0 flex items-center justify-end gap-0.5">
              {floor.alarmCritical > 0 && (
                <span
                  className="inline-block w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse"
