...
export default function AlarmHistoryPage() {
  ...
  const downloadCSV = () => {
    ...
    const headers = ["ID", "심각도", "장비", "유형", "메시지", "값", "임계값", "발생", "해소", "확인"];
  ...
  return (
    <div className="min-h-screen">
      <Header title="알람 이력" description="전체 알람 아카이브 조회" />
          ...
          <Card>
            <CardContent className="py-4 px-4">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3">
                ...
                >
                  {suppressing ? <Loader2 className="h-3 w-3 animate-spin" /> : "억제"}
                </Button>
              </div>
            </CardContent>
          </Card>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      ...
                      <th className="text-left py-2.5 px-3 text-gray-500 font-medium">유형</th>
                      <th className="text-right py-2.5 px-3 text-gray-500 font-medium">값/임계값</th>
                      <th className="text-left py-2.5 px-3 text-gray-500 font-medium">발생</th>
                      <th className="text-left py-2.5 px-3 text-gray-500 font-medium">해소</th>
                      <th className="text-left py-2.5 px-3 text-gray-500 font-medium">확인</th>
                      <th className="text-center py-2.5 px-3 text-gray-500 font-medium">조치</th>
                    </tr>
                  </thead>
                  <tbody>
                    {alarms.map((alarm) => (
                      <tr key={alarm.id} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-2 px-3">{getSeverityBadge(alarm.severity)}</td>
