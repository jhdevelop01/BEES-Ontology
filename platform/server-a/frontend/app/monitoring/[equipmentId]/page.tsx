...
function getGaugeColor(pointId: string, value: number): string {
  ...
  if (pid.includes("temp")) {
    if (value < 18 || value > 30) return "text-rose-400";
    if (value < 20 || value > 28) return "text-amber-400";
    return "text-emerald-400";
  }
  if (pid.includes("humid") || pid.includes("rh")) {
    if (value < 30 || value > 70) return "text-rose-400";
    if (value < 40 || value > 60) return "text-amber-400";
    return "text-emerald-400";
  }
  if (pid.includes("co2")) {
    if (value > 1500) return "text-rose-400";
...
function getSeverityBadge(severity: string) {
  switch (severity) {
    ...
    case "major":
      return <Badge className="bg-orange-500/10 text-orange-400 border-orange-500/20">major</Badge>;
    case "minor":
      return <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/20">minor</Badge>;
    default:
...
// 차트 색상 팔레트
const CHART_COLORS = [
  "#22d3ee", "#34d399", "#fbbf24", "#818cf8", "#f472b6",
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-lg font-medium text-rose-400">{error || t("equipmentNotFound")}</p>
          <button
            onClick={() => router.push("/monitoring")}
            className="mt-4 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-lg hover:shadow-glow"
          >
            {t("backToMonitoring")}
          </button>
        </div>
      ...
      <div className="p-3 md:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-white">{getDisplayName(locale, detail.label, detail.name)}</h2>
            ...
            <p className="text-xs text-slate-500 font-mono">{detail.name}</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {sensorPointIds.map((pid, i) => {
            ...
            const shortName = pid.replace("bldg:", "").replace(/_/g, " ");
            const gaugeColor = value !== null ? getGaugeColor(pid, value) : "text-slate-500";
            ...
            return (
              <Card key={pid}>
                <CardContent className="pt-3 pb-2 px-3">
                  ...
                  <p className="text-[11px] text-slate-400 truncate" title={shortName}>
                    {shortName}
                  </p>
                  <p className={`text-xl font-bold font-mono mt-0.5 ${gaugeColor}`}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thSensor")}</th>
                        <th className="text-right py-2 px-2 text-slate-400 font-medium text-xs">{t("thAvg")}</th>
                        <th className="text-right py-2 px-2 text-slate-400 font-medium text-xs">{t("thMin")}</th>
                        <th className="text-right py-2 px-2 text-slate-400 font-medium text-xs">{t("thMax")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {performance.sensors.map((s) => (
                        <tr key={s.id} className="border-b border-white/5">
                          <td className="py-1.5 px-2 text-xs truncate max-w-[150px] text-slate-300" title={s.id}>
                            {s.id.replace(/_/g, " ")}
                          </td>
                          <td className="py-1.5 px-2 text-right font-semibold font-mono text-xs text-white">{s.avg}</td>
                          <td className="py-1.5 px-2 text-right text-xs text-cyan-400">
                            <TrendingDown className="h-3 w-3 inline mr-0.5" />
                            {s.min}
                          </td>
                          <td className="py-1.5 px-2 text-right text-xs text-rose-400">
                  ...
                  {performance.uptime_pct !== null && (
                    <div className="mt-3 flex items-center gap-2 text-sm">
                      ...
                      <span className="text-slate-300">{t("uptimeRate")} <strong className="text-white">{performance.uptime_pct}%</strong></span>
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-sm text-slate-500 text-center py-6">{t("noPerformanceData")}</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {detail.connections.length > 0 ? (
                  ...
                  {detail.connections
                    ...
                    .map((c, i) => (
                      ...
                      >
                        ...
                        <span className="truncate text-slate-300" title={c.target_id}>
                          {c.target_name}
                        </span>
                ...
                <p className="text-sm text-slate-500 text-center py-6">{t("noConnections")}</p>
              )}
            </CardContent>
          </Card>
        </div>
            ...
            {alarms && alarms.items.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10">
                      <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thTime")}</th>
                      <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thSeverity")}</th>
                      <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thType")}</th>
                      <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thMessage")}</th>
                      <th className="text-left py-2 px-2 text-slate-400 font-medium text-xs">{t("thStatus")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {alarms.items.slice(0, 20).map((a) => (
                      <tr key={a.id} className="border-b border-white/5 hover:bg-white/5">
                        <td className="py-1.5 px-2 text-xs text-slate-400 whitespace-nowrap">
                        ...
                        <td className="py-1.5 px-2 text-xs text-slate-300">{a.alarm_type}</td>
                        <td className="py-1.5 px-2 text-xs text-slate-400 max-w-[300px] truncate">
                          {a.message}
                        </td>
              ...
              <p className="text-sm text-slate-500 text-center py-6">{t("noAlarmHistory")}</p>
            )}
          </CardContent>
        </Card>
      </div>
