...
| 파일 | 위치 | 설명 |
|------|------|------|
| GEC_B_Ontology.ttl | `ontology/` | 메인 온톨로지 (v2.2.1, ~14,425줄, 11,470 트리플, 1,732 인스턴스) |
...
**Phase 2~4.5 (유지)**: 284개 장비 + 670개 포인트 실시간 시뮬레이션, 온톨로지 그래프, 토폴로지, LLM 채팅, InfluxDB, 알람, Grafana, BACnet, JWT 인증, 반응형 디자인, 시나리오/고장주입, 열역학 모델, 데이터 품질
...
curl -s http://localhost:8010/api/stream/snapshot        # 데이터 확인 (670포인트 기대)
